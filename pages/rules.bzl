def _assemble_react_impl(ctx):
    ...
    ctx.actions.run_shell(
        ...
        command = "cd {} && ../{} --mode {} && tar -cvzf ../{} ./pages/".format(
            ctx.attr.package_dir, ctx.executable._react_cli_wrapper_binary.path, ctx.attr.react_build_mode, archive.path),
        tools = [ctx.executable._react_cli_wrapper_binary],
        progress_message = "Running webpack (build) in {}".format(ctx.attr.package_dir),
        mnemonic = "AssembleVue",
