...
RELEASES=$(curl -s https://api.github.com/repos/mindmapsdb/mindmapsdb/releases | grep browser_download_url | grep tgz | awk '{print $2}' | sed -e 's/\"//g')
...
for RELEASE in $RELEASES; do
  VERSION=$(basename ${RELEASE} | sed -e 's/mindmaps-dist-//' -e 's/\.tgz//')
  if [[ ! -d v${VERSION} ]]; then
    echo "Downloading ${VERSION}"
    wget --continue -q ${RELEASE} -o mindmaps-dist-${VERSION}.tgz
    mkdir -p v${VERSION}
    tar -xf mindmaps-dist-${VERSION}.tgz -C v${VERSION} mindmaps-dist-${VERSION}/docs/api/ --strip-components=3
    rm mindmaps-dist-${VERSION}.tgz
  fi
done
