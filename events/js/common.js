...
const populateSpeakerModal = (speaker, sessions, speakers) => {
    ...
    if (url) {
      let linkHtml = `
            <a href="PLACEHOLDER_ADDRESS" target="_blank">
                <div class="logo-wrapper"><img src="/events/img/icons/PLACEHOLDER_ICON" /></div>
            </a>`;
  ...
  if (companyLogoFileName.length) {
    $("#company img").attr("src", `/events/img/companies/${companyLogoFileName}`);
  }
...
const loadSpeakerCompanyLogo = (speaker) => {
  ...
  companyLogo.src = "/events/img/companies/" + companyLogoFileName;
