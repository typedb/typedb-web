...
window.MNDMPS.Splitter={_data:{transitionXRegex:/\.*translateX\((.*)px\)/i},moveClip:function(obj){obj.splitter.setAttribute("data-clip",obj.pos);if(document.documentElement.style.hasOwnProperty("webkitClipPath"))obj.splitter.style.webkitClipPath="inset(0px 0px 0px "+obj.pos+"px)";else if(obj.splitter.view)obj.splitter.style.clip="rect(0px, "+obj.splitter.view.offsetWidth+"px, "+obj.splitter.view.offsetHeight+"px, "+obj.pos+"px)"},moveKnob:function(obj){obj.knob.setAttribute("data-translate",obj.pos);
obj.knob.style.transform="translateX("+obj.pos+"px)"},setNewContainerWidth:function(splitter){splitter.view.setAttribute("data-width",splitter.view.offsetWidth)},getChangePercent:function(splitter){var width=parseInt(splitter.view.getAttribute("data-width"),10),newWidth=splitter.view.offsetWidth;return(newWidth-width)/width},getNewPos:function(splitter,oldPos,changePercent){var newPos=null;if(changePercent<0)newPos=oldPos+Math.floor(oldPos*changePercent);else newPos=oldPos+Math.ceil(oldPos*changePercent);
return newPos},resizeSplitters:function(){var _this=window.MNDMPS.Splitter,data=_this._data,splitters=data.splitters,changePercent=null,dragPos=null;for(var i=0;i<splitters.length;i++){dragPos=splitters[i].dragger.getAttribute("data-translate");if(dragPos===null||parseInt(dragPos,10)<=10)continue;changePercent=_this.getChangePercent(splitters[i]);_this.moveKnob({knob:splitters[i].dragger,pos:_this.getNewPos(splitters[i],parseInt(dragPos,10),changePercent)});for(var j=0;j<splitters[i].slides.length;j++)_this.moveClip({splitter:splitters[i].slides[j].children[1],
pos:_this.getNewPos(splitters[i],parseInt(splitters[i].slides[j].children[1].getAttribute("data-clip"),10),changePercent)});_this.setNewContainerWidth(splitters[i])}},processSlides:function(splitter){var data=this._data,width=splitter.view.offsetWidth,height=splitter.view.offsetHeight,el=null;for(var i=0;i<splitter.slides.length;i++){el=splitter.slides[i].querySelectorAll('[data-slide="right"]')[0];if(document.documentElement.style.hasOwnProperty("webkitClipPath"))el.style.webkitClipPath="inset(0px 0px 0px "+
data.clipX+"px)";else el.style.clip="rect(0px, "+width+"px, "+height+"px, "+data.clipX+"px)";el.setAttribute("data-clip",data.clipX)}},initDragger:function(obj){var _this=this,data=this._data,dragger=obj.dragger,view=obj.view;function removeListeners(event){document.removeEventListener("mousedown",removeListeners,false);document.removeEventListener("touchstart",removeListeners,false);document.removeEventListener("mouseup",removeListeners,false);document.removeEventListener("touchend",removeListeners,
false);document.removeEventListener("mousemove",drag,false);document.removeEventListener("touchmove",drag,false)}function drag(event){window.MNDMPS.Helpers.pauseEvent(event);var newMouseX=null;if(event.touches&&event.touches.length)newMouseX=event.touches[0].pageX;else newMouseX=event.pageX;var diff=newMouseX-data.mouseX,relativeOffsetX=data.relativeX+diff+data.offsetX,newTranslateX=null;if(relativeOffsetX<=5||relativeOffsetX>=data.viewWidth-5)return;if(data.translateX)newTranslateX=data.translateX+
diff;else newTranslateX=diff;dragger.style.transform="translateX("+newTranslateX+"px)";dragger.setAttribute("data-translate",newTranslateX);data.clipX=relativeOffsetX;_this.processSlides(obj)}function startDrag(event){if(obj.automoveHandler)_this.cancelAutomove(obj);if(event.touches&&event.touches.length)data.mouseX=event.touches[0].pageX;else data.mouseX=event.pageX;data.viewX=window.MNDMPS.Helpers.getPosition(view).x;data.viewWidth=view.offsetWidth;data.relativeX=data.mouseX-data.viewX;data.translateX=
data.transitionXRegex.exec(dragger.getAttribute("style"));data.offsetX=window.MNDMPS.Helpers.getPosition(event.target).x-data.mouseX;if(data.translateX){data.translateX=parseInt(data.translateX[1],10);data.offsetX+=data.translateX}document.addEventListener("mousemove",drag,false);document.addEventListener("touchmove",drag,false);document.addEventListener("mouseup",removeListeners,false);document.addEventListener("touchend",removeListeners,false);setTimeout(function(){document.addEventListener("mousedown",
removeListeners,false);document.addEventListener("touchstart",removeListeners,false)},0)}dragger.children[0].addEventListener("mousedown",startDrag,false);dragger.children[0].addEventListener("touchstart",startDrag,false)},create:function(el){var data=this._data,splitter={view:el,dragger:el.querySelectorAll('[data-splitter="drag"]')[0],slides:el.querySelectorAll('[data-splitter="slides"]')[0].children},initialPercent=5,initialPos=null,slides=null;data.clipX=0;splitter.view.setAttribute("data-width",
splitter.view.offsetWidth);this.initDragger(splitter);this.processSlides(splitter);if(!data.splitters)data.splitters=[];data.splitters.push(splitter);initialPos=Math.floor(splitter.view.offsetWidth/100*initialPercent);slides=splitter.slides;window.MNDMPS.Splitter.moveKnob({knob:splitter.dragger,pos:initialPos});for(var i=0;i<slides.length;i++)window.MNDMPS.Splitter.moveClip({splitter:slides[i].children[1],pos:initialPos});return splitter},cancelAutomove:function(splitter){window.removeEventListener("scroll",
splitter.automoveHandler,false)},initAutomove:function(splitter){splitter.automoveHandler=function(){var view=splitter.view,viewOffset=view.getBoundingClientRect(),windowPercent=window.innerHeight/100,scrolledPercent=100-(viewOffset.top+view.offsetHeight/2)/windowPercent,newPos=Math.floor(view.offsetWidth/100*scrolledPercent),slides=splitter.slides;if(scrolledPercent<=5||scrolledPercent>=95)return;window.MNDMPS.Splitter.moveKnob({knob:splitter.dragger,pos:newPos});for(var i=0;i<slides.length;i++)window.MNDMPS.Splitter.moveClip({splitter:slides[i].children[1],
pos:newPos})};window.addEventListener("scroll",splitter.automoveHandler,false)},init:function(){window.addEventListener("resize",this.resizeSplitters,false)}};var _self=typeof window!=="undefined"?window:typeof WorkerGlobalScope!=="undefined"&&self instanceof WorkerGlobalScope?self:{};
...
window.MNDMPS.Slider={_data:{minTabOpacity:.4,tabSpeed:200},updateParams:function(){var _this=window.MNDMPS.Slider,data=_this._data;data.scrollerWidth=data.wrapper.offsetWidth;data.contentWidth=data.scrollerWidth*data.tabs.children.length;data.containerPercent=data.scrollerWidth/100;data.scrolledPercent=data.contentWidth/100;_this.processScroll()},processScroll:function(){var _this=window.MNDMPS.Slider,data=_this._data,knobPosition=data.wrapper.scrollLeft/data.scrolledPercent;data.scrollKnob.style.left=
knobPosition+"%";_this.processTabs(knobPosition)},switchTab:function(event){var _this=window.MNDMPS.Slider,data=_this._data,tabs=[].slice.call(data.tabs.children),index=tabs.indexOf(event.target),scrollPos=index*data.scrollerWidth;data.tabTween.stop();data.tabTween.tween({from:{x:data.wrapper.scrollLeft},to:{x:scrollPos},duration:data.tabSpeed,easing:"easeInOutQuart",step:function(state){data.wrapper.scrollLeft=state.x},finish:function(){data.wrapper.scrollLeft=scrollPos}})},processTabs:function(pos){var data=
this._data,tabs=data.tabs.children,value=pos/data.scrollKnobWidth,index={prev:parseInt(value,10),next:Math.ceil(value)},opacity={prev:index.next-value,next:value-index.prev};if(index.prev===index.next)opacity.prev=opacity.next=1;for(var i=0;i<tabs.length;i++)tabs[i].removeAttribute("style");for(var key in index)if(tabs[index[key]])tabs[index[key]].style.opacity=Math.max(opacity[key],data.minTabOpacity)},init:function(){var data=this._data;data.scroller=document.getElementsByClassName("promoSlider")[0];
data.container=data.scroller.children[0];data.wrapper=data.container.children[0];data.controls=data.scroller.children[1];data.scrollbar=data.controls.children[0];data.scrollKnob=data.scrollbar.children[0];data.tabs=data.controls.children[1];data.scrollKnobWidth=100/data.tabs.children.length;data.scrollKnob.style.width=data.scrollKnobWidth+"%";data.scrollerWidth=data.wrapper.offsetWidth;data.contentWidth=data.scrollerWidth*data.tabs.children.length;data.containerPercent=data.scrollerWidth/100;data.scrolledPercent=
data.contentWidth/100;data.wrapper.addEventListener("scroll",this.processScroll,false);data.tabs.addEventListener("click",this.switchTab,false);window.addEventListener("resize",this.updateParams,false);data.tabTween=new Tweenable;this.processScroll()}};window.MNDMPS=window.MNDMPS||{};
...
false);window.addEventListener("scroll",this.checkPosition,false);window.addEventListener("resize",this.checkWidth,false);this.checkWidth()}};window.MNDMPS=window.MNDMPS||{};
