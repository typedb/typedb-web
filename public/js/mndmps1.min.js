...
this.getBiggerSide(graphName)/100*svgElement.r/8;break;case "Edge":fontSize=this.getBiggerSide(graphName)/100*1.3;break}if(fontSize>data["max"+type+"Font"])fontSize=data["max"+type+"Font"];newStyle.push("font-size: "+fontSize+"px;");if(postRender&&fontSize<data["min"+type+"Font"])newStyle.push("display: none;");return newStyle.join(" ")},redraw:function(pointer){var _this=this,data=this._data,graphs=this._data.graphs,graph=null;for(var key in graphs){if(pointer&&pointer!==key)continue;graph=graphs[key];
...
100*d.source.cx,y:data.graphs[graphName].height/100*d.source.cy},target={x:data.graphs[graphName].width/100*d.target.cx,y:data.graphs[graphName].height/100*d.target.cy};return Math.sqrt(Math.pow(source.x-target.x,2)+Math.pow(source.y-target.y,2))}).charge(0).gravity(newGraph,0).start();newGraph.edges=newGraph.svg.selectAll("line").data(dataset.edges).enter().append("line").attr({"id":function(d,i){return graphName+"_edge_"+i},"marker-end":function(d){return"url(#"+graphName+"_"+(d.type||"default")+
"-arrowhead)"},"style":function(d){return"stroke: "+data.colors[d.type||"default"]}});newGraph.nodes=newGraph.svg.selectAll("circle").data(dataset.nodes).enter().append("circle").attr({"r":function(d){return _this.getBiggerSide(graphName)/100*(d.r/2)},"style":function(d){return"stroke: "+data.colors[d.type]+"; "+"fill: "+data.colors["dark-bg"]}}).call(newGraph.force.drag);newGraph.nodelabels=newGraph.svg.selectAll(".nodelabel").data(dataset.nodes).enter().append("text").attr({"text-anchor":"middle",
