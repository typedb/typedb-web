...
window.MNDMPS.Splitter={_data:{transitionXRegex:/\.*translateX\((.*)px\)/i},resizeSplitters:function(){var data=window.MNDMPS.Splitter._data,splitters=data.splitters,changePercent=null,width=null,newHeight=null,newWidth=null,dragPos=null,newDragPos=null,clipPos=null,newClipPos=null;for(var i=0;i<splitters.length;i++){dragPos=splitters[i].dragger.getAttribute("data-translate");if(dragPos===null||parseInt(dragPos,10)<=10)continue;width=parseInt(splitters[i].view.getAttribute("data-width"),10);newWidth=
splitters[i].view.offsetWidth;newHeight=splitters[i].view.offsetHeight;changePercent=(newWidth-width)/width;splitters[i].view.setAttribute("data-width",newWidth);dragPos=parseInt(dragPos,10);if(changePercent<0)newDragPos=dragPos+Math.floor(dragPos*changePercent);else newDragPos=dragPos+Math.ceil(dragPos*changePercent);splitters[i].dragger.setAttribute("data-translate",newDragPos);splitters[i].dragger.style.transform="translateX("+newDragPos+"px)";for(var j=0;j<splitters[i].slides.length;j++){clipPos=
parseInt(splitters[i].slides[j].children[1].getAttribute("data-clip"),10);if(changePercent<0)newClipPos=clipPos+Math.floor(clipPos*changePercent);else newClipPos=clipPos+Math.ceil(clipPos*changePercent);splitters[i].slides[j].children[1].setAttribute("data-clip",newClipPos);if(document.documentElement.style.hasOwnProperty("webkitClipPath"))splitters[i].slides[j].children[1].style.webkitClipPath="inset(0px 0px 0px "+newClipPos+"px)";else splitters[i].slides[j].children[1].style.clip="rect(0px, "+newWidth+
"px, "+newHeight+"px, "+newClipPos+"px)"}}},processSlides:function(splitter){var data=this._data,width=splitter.view.offsetWidth,height=splitter.view.offsetHeight,el=null;for(var i=0;i<splitter.slides.length;i++){el=splitter.slides[i].querySelectorAll('[data-slide="right"]')[0];if(document.documentElement.style.hasOwnProperty("webkitClipPath"))el.style.webkitClipPath="inset(0px 0px 0px "+data.clipX+"px)";else el.style.clip="rect(0px, "+width+"px, "+height+"px, "+data.clipX+"px)";el.setAttribute("data-clip",
data.clipX)}},initDragger:function(obj){var _this=this,data=this._data,dragger=obj.dragger,view=obj.view;function removeListeners(event){document.removeEventListener("mousedown",removeListeners,false);document.removeEventListener("touchstart",removeListeners,false);document.removeEventListener("mouseup",removeListeners,false);document.removeEventListener("touchend",removeListeners,false);document.removeEventListener("mousemove",drag,false);document.removeEventListener("touchmove",drag,false)}function drag(event){window.MNDMPS.Helpers.pauseEvent(event);
var newMouseX=null;if(event.touches&&event.touches.length)newMouseX=event.touches[0].pageX;else newMouseX=event.pageX;var diff=newMouseX-data.mouseX,relativeOffsetX=data.relativeX+diff+data.offsetX,newTranslateX=null;if(relativeOffsetX<=5||relativeOffsetX>=data.viewWidth-5)return;if(data.translateX)newTranslateX=data.translateX+diff;else newTranslateX=diff;dragger.style.transform="translateX("+newTranslateX+"px)";dragger.setAttribute("data-translate",newTranslateX);data.clipX=relativeOffsetX;_this.processSlides(obj)}
function startDrag(event){if(event.touches&&event.touches.length)data.mouseX=event.touches[0].pageX;else data.mouseX=event.pageX;data.viewX=window.MNDMPS.Helpers.getPosition(view).x;data.viewWidth=view.offsetWidth;data.relativeX=data.mouseX-data.viewX;data.translateX=data.transitionXRegex.exec(dragger.getAttribute("style"));data.offsetX=window.MNDMPS.Helpers.getPosition(event.target).x-data.mouseX;if(data.translateX){data.translateX=parseInt(data.translateX[1],10);data.offsetX+=data.translateX}document.addEventListener("mousemove",
drag,false);document.addEventListener("touchmove",drag,false);document.addEventListener("mouseup",removeListeners,false);document.addEventListener("touchend",removeListeners,false);setTimeout(function(){document.addEventListener("mousedown",removeListeners,false);document.addEventListener("touchstart",removeListeners,false)},0)}dragger.children[0].addEventListener("mousedown",startDrag,false);dragger.children[0].addEventListener("touchstart",startDrag,false)},create:function(el){var data=this._data,
splitter={view:el,dragger:el.querySelectorAll('[data-splitter="drag"]')[0],slides:el.querySelectorAll('[data-splitter="slides"]')[0].children};data.clipX=0;splitter.view.setAttribute("data-width",splitter.view.offsetWidth);this.initDragger(splitter);this.processSlides(splitter);if(!data.splitters)data.splitters=[];data.splitters.push(splitter)},init:function(){window.addEventListener("resize",this.resizeSplitters,false)}};var _self=typeof window!=="undefined"?window:typeof WorkerGlobalScope!=="undefined"&&self instanceof WorkerGlobalScope?self:{};
...
window.MNDMPS.Graph={_data:{graphs:{},maxNodeRadius:60,nodePadding:5,colors:{"default":"#bbbcbc","instance":"#a1d884","concept-type":"#ff7878","relation":"#77dd77","relation-type":"#bfc0d1","resource":"#a1d884","resource-type":"#ff7878","role-type":"#ffb96d","meta":"#5bc2e7","dark-bg":"#383838","light-bg":"#ffffff"}},redraw:function(pointer){var _this=this,graphs=this._data.graphs,graph=null;for(var key in graphs){if(pointer&&pointer!==key)continue;graph=graphs[key];if(!graph.nodeLabelsInitialised)setTimeout(function(){graph.svg.selectAll(".nodelabel").call(_this.wrap);
graph.nodeLabelsInitialised=true},25);graph.width=graph.container[0][0].offsetWidth;graph.height=graph.container[0][0].offsetHeight;graph.container.select("svg").attr({"width":graph.width,"height":graph.height});graph.force.size([graph.width,graph.height]).start()}},offsetEdge:function(graph,d){var data=this._data,side=graph.width>graph.height?graph.width:graph.height,sourceR=side/100*d.source.r/2,targetR=side/100*d.target.r/2,sourceCirc=sourceR*2*Math.PI,targetCirc=targetR*2*Math.PI,stRatio=sourceCirc/
targetCirc,diffX=d.target.y-d.source.y,diffY=d.target.x-d.source.x,angle=Math.atan2(diffY,diffX)+Math.PI/2,x1=d.source.x-sourceR*Math.cos(angle),y1=d.source.y+sourceR*Math.sin(angle),x2=d.target.x+targetR*Math.cos(angle),y2=d.target.y-targetR*Math.sin(angle);return{x1:x1,y1:y1,x2:x2,y2:y2}},gravity:function(graph,alpha){var data=this._data;return function(d){d.x+=(graph.width/100*d.cx-d.x)*alpha;d.y+=(graph.height/100*d.cy-d.y)*alpha}},collide:function(graph,alpha){var data=this._data,quadtree=d3.geom.quadtree(graph.nodes);
return function(d){var r=d.radius+data.maxNodeRadius+data.nodePadding,nx1=d.x-r,nx2=d.x+r,ny1=d.y-r,ny2=d.y+r;quadtree.visit(function(quad,x1,y1,x2,y2){if(quad.point&&quad.point!==d){var x=d.x-quad.point.x,y=d.y-quad.point.y,l=Math.sqrt(Math.pow(x,2)+Math.pow(y,2)),r=d.radius+quad.point.radius+(d.color!==quad.point.color)*data.nodePadding;if(l<r){l=(l-r)/(l*alpha);d.x-=x*=l;d.y-=y*=l;quad.point.x+=x;quad.point.y+=y}}return x1>nx2||x2<nx1||y1>ny2||y2<ny1})}},wrap:function(text){text.each(function(){var text=
d3.select(this),width=parseInt(text.attr("lable-width"),10),words=text.text().split(/\s+/).reverse(),word=null,line=[],lineNumber=0,lineHeight=1,y=text.attr("y"),dy=0,tspan=text.text(null).append("tspan").attr({"x":0,"y":0,"dy":++lineNumber*lineHeight+dy+"em"});if(words.length===1){tspan.text(words[0]);return}while(word=words.pop()){line.push(word);tspan.text(line.join(" "));if(tspan.node().getComputedTextLength()>width){line.pop();tspan.text(line.join(" "));line=[word];tspan=text.append("tspan").attr({"x":0,
"y":0,"dy":++lineNumber*lineHeight+dy+"em"}).text(word)}}})},getBiggerSide:function(name){var data=this._data,graph=data.graphs[name];return graph.width>graph.height?graph.width:graph.height},addArrowheads:function(graphName,types){var data=this._data,graph=data.graphs[graphName];graph.defs=graph.svg.append("defs");for(var i=0;i<types.length;i++)graph.defs.append("marker").attr({"id":graphName+"_"+types[i]+"-arrowhead","viewBox":"-0 -5 10 10","refX":7,"refY":0,"orient":"auto","markerWidth":10,"markerHeight":10,
"xoverflow":"visible"}).append("svg:path").attr("d","M 0,-5 L 7,0 L 0,5").attr("stroke",data.colors[types[i]])},init:function(htmlNode,dataset){var _this=this,data=this._data,newGraph=null,graphName="graph"+Object.keys(data.graphs).length;data.graphs[graphName]={};newGraph=data.graphs[graphName];newGraph.nodeLabelsInitialised=false;newGraph.container=d3.select(htmlNode);newGraph.width=newGraph.container[0][0].offsetWidth;newGraph.height=newGraph.container[0][0].offsetHeight;newGraph.svg=newGraph.container.append("svg").attr({"width":newGraph.width,
"height":newGraph.height});newGraph.force=d3.layout.force().nodes(dataset.nodes).links(dataset.edges).size([newGraph.width,newGraph.height]).linkDistance(function(d){var source={x:data.graphs[graphName].width/100*d.source.cx,y:data.graphs[graphName].height/100*d.source.cy},target={x:data.graphs[graphName].width/100*d.target.cx,y:data.graphs[graphName].height/100*d.target.cy};return Math.sqrt(Math.pow(source.x-target.x,2)+Math.pow(source.y-target.y,2))}).charge(0).gravity(newGraph,0).start();newGraph.edges=
newGraph.svg.selectAll("line").data(dataset.edges).enter().append("line").attr("id",function(d,i){return graphName+"_edge_"+i}).attr("marker-end",function(d){return"url(#"+graphName+"_"+(d.type||"default")+"-arrowhead)"}).style("stroke",function(d){return data.colors[d.type||"default"]});newGraph.nodes=newGraph.svg.selectAll("circle").data(dataset.nodes).enter().append("circle").attr({"r":function(d){return _this.getBiggerSide(graphName)/100*(d.r/2)}}).style("stroke",function(d,i){return data.colors[d.type]}).style("fill",
data.colors["dark-bg"]).call(newGraph.force.drag);newGraph.nodelabels=newGraph.svg.selectAll(".nodelabel").data(dataset.nodes).enter().append("text").attr({"text-anchor":"middle","x":0,"y":0,"class":"nodelabel","fill":function(d){return data.colors[d.type]},"font-size":function(d){return _this.getBiggerSide(graphName)/100*d.r/8},"lable-width":function(d){return _this.getBiggerSide(graphName)/100*d.r/Math.sqrt(2)}}).text(function(d){return d.text});newGraph.edgepaths=newGraph.svg.selectAll(".edgepath").data(dataset.edges).enter().append("path").attr({"d":function(d){return"M "+
d.source.x+" "+d.source.y+" L "+d.target.x+" "+d.target.y},"class":"edgepath","id":function(d,i){return graphName+"_edgepath_"+i}});newGraph.edgelabels=newGraph.svg.selectAll(".edgelabel").data(dataset.edges).enter().append("text").attr({"class":"edgelabel","id":function(d,i){return graphName+"_edgelabel_"+i},"dx":0,"dy":-5,"fill":function(d){return data.colors[d.type||"default"]}});newGraph.edgelabels.append("textPath").attr("xlink:href",function(d,i){return"#"+graphName+"_edgepath_"+i}).attr("startOffset",
"50%").attr("text-anchor","middle").text(function(d,i){return d.text});this.addArrowheads(graphName,["default","relation"]);setTimeout(function(){if(htmlNode.offsetWidth>0||htmlNode.offsetHeight>0){newGraph.svg.selectAll(".nodelabel").call(_this.wrap);newGraph.nodeLabelsInitialised=true}},25);newGraph.force.on("tick",function(e){newGraph.nodes.each(_this.gravity(newGraph,.2*e.alpha)).each(_this.collide(newGraph,.5)).attr("r",function(d){return _this.getBiggerSide(graphName)/100*(d.r/2)}).attr("cx",
function(d){return d.x}).attr("cy",function(d){return d.y});newGraph.edges.each(function(d){var altCoords=_this.offsetEdge(newGraph,d);d3.select(this).attr("x1",altCoords.x1).attr("y1",altCoords.y1).attr("x2",altCoords.x2).attr("y2",altCoords.y2)});newGraph.nodelabels.attr({"transform":function(d){return"translate("+d.x+","+(d.y-this.getBoundingClientRect().height/2)+")"},"font-size":function(d){return _this.getBiggerSide(graphName)/100*d.r/8},"style":function(d){if(!newGraph.nodeLabelsInitialised)return"";
var fontSize=_this.getBiggerSide(graphName)/100*d.r/8;return fontSize<8?"display: none;":""}});newGraph.edgepaths.attr("d",function(d){var altCoords=_this.offsetEdge(newGraph,d),path="M "+altCoords.x1+" "+altCoords.y1+" L "+altCoords.x2+" "+altCoords.y2;return path});newGraph.edgelabels.attr({"transform":function(d,i){if(d.target.x<d.source.x){var bbox=this.getBBox(),rx=bbox.x+bbox.width/2,ry=bbox.y+bbox.height/2;return"rotate(180 "+rx+" "+ry+")"}else return"rotate(0)"}})})}};window.MNDMPS=window.MNDMPS||{};
...
function listenerForScroll(input){input.addEventListener("scroll",function(event){$(this).next()[0].scrollTop=this.scrollTop},false)}for(var i=0;i<textareas.length;i++){renderOutput(textareas[i]);listenerForScroll(textareas[i])}},init:function(){var data=this._data,codeEditor=document.getElementsByClassName("editor")[0];this.typist();window.MNDMPS.Splitter.create(document.getElementById("code-container").querySelectorAll('[data-splitter="view"]')[0]);window.MNDMPS.Splitter.init();window.MNDMPS.Tabs.init(document.getElementById("code-container").querySelectorAll('[data-tabs="view"]')[0]);
this.initPrism();var graqlSvgContainers=document.querySelectorAll('[data-splitter="slides"]')[0].querySelectorAll('[data-slide="right"]');for(var i=0;i<graqlSvgContainers.length;i++)if(window.MNDMPS.Graph._data.homepageGraphs[i])window.MNDMPS.Graph.init(graqlSvgContainers[i],window.MNDMPS.Graph._data.homepageGraphs[i]);window.addEventListener("resize",function(){if(codeEditor){var tabs=codeEditor.getElementsByClassName("content")[0].children;for(var i=0;i<tabs.length;i++)if(tabs[i].classList.contains("active")){window.MNDMPS.Graph.redraw("graph"+
i);break}}},false)}};window.MNDMPS.App.init();
