...
event.cancelBubble=true;event.returnValue=false;return false},dontScrollParent:function(el,turnOff){function catchScroll(ev){var scrollTop=this.scrollTop,scrollHeight=this.scrollHeight,height=this.offsetHeight,delta=ev.wheelDelta,up=delta>0;function prevent(){ev.stopPropagation();ev.preventDefault();ev.returnValue=false;return false}if(event.type==="touchmove")return prevent();else if(!up&&-delta>scrollHeight-height-scrollTop){this.scrollTop=scrollHeight;return prevent()}else if(up&&delta>scrollTop){this.scrollTop=
0;return prevent()}}el.removeEventListener("touchmove",catchScroll,false);el.removeEventListener("DOMMouseScroll",catchScroll,false);el.removeEventListener("mousewheel",catchScroll,false);if(!turnOff){el.addEventListener("touchmove",catchScroll,false);el.addEventListener("DOMMouseScroll",catchScroll,false);el.addEventListener("mousewheel",catchScroll,false)}}};window.MNDMPS=window.MNDMPS||{};
...
lookbehind:true}});Prism.languages.insertBefore("java","function",{"annotation":{alias:"punctuation",pattern:/(^|[^.])@\w+/,lookbehind:true}});(function(){Prism.languages.graql={comment:{pattern:/#.*/,alias:"comment"},string:{pattern:/".*?"/,alias:"string"},keyword:{pattern:/(select|ask|insert|delete|where|isa|ako|plays-role|has-role|has-scope|datatype|is-abstract|has|value|id|of|limit|offset|order|by)(?![-\$a-zA-Z_0-9])/,alias:"keyword"},special:{pattern:/graql>|results>|\.\.\./},month:{pattern:/Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec(?!{-\$a-zA-Z_0-9})/,alias:"number"},variable:{pattern:/\$[-a-zA-Z_0-9]+(?![-\$a-zA-Z_0-9])/,alias:"variable"},
type:{pattern:/[-a-zA-Z_][-a-zA-Z_0-9]*(?![-\$a-zA-Z_0-9])/,alias:"function"},number:{pattern:/[0-9]+(\.[0-9][0-9]*)?/,alias:"number"},operator:{pattern:/=|!=|>|<|>=|<=|contains|regex/,alias:"operator"}}})();window.MNDMPS=window.MNDMPS||{};
...
false);window.addEventListener("scroll",this.checkPosition,false);window.addEventListener("resize",this.checkWidth,false);window.MNDMPS.Helpers.dontScrollParent(data.menu)}};window.MNDMPS=window.MNDMPS||{};
window.MNDMPS.App={_data:{},typist:function(){var data=this._data;data.typist=$("#typist");data.typist.typed({strings:["a knowledge graph","an inference engine","a graph analytics tool","a distributed semantic database"],typeSpeed:10,backSpeed:5,backDelay:1500,loop:false})},initPrism:function(){var textareas=document.getElementsByClassName("code-input");function renderOutput(input){var value=input.value;value=value.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")+"\n";$(input).next().children(0)[0].innerHTML=
...
if(codeEditor)window.addEventListener("resize",function(){var tabs=codeEditor.getElementsByClassName("content")[0].children;for(var i=0;i<tabs.length;i++)if(tabs[i].classList.contains("active")){window.MNDMPS.Graph.redraw("graph"+i);break}},false);window.MNDMPS.Slider.init()}};window.MNDMPS.App.init();
