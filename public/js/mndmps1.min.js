...
"x":0,"y":0,"class":"nodelabel","fill":function(d){return data.colors[d.type]},"lable-width":function(d){return _this.getBiggerSide(graphName)/100*d.r/Math.sqrt(2)},"style":function(d){return _this.resizeFont(graphName,d,"Node",false)}}).text(function(d){return d.text});newGraph.edgepaths=newGraph.svg.selectAll(".edgepath").data(dataset.edges).enter().append("path").attr({"d":function(d){return"M "+d.source.x+" "+d.source.y+" L "+d.target.x+" "+d.target.y},"class":"edgepath","id":function(d,i){return graphName+
"_edgepath_"+i}});newGraph.edgelabels=newGraph.svg.selectAll(".edgelabel").data(dataset.edges).enter().append("text").attr({"class":"edgelabel","id":function(d,i){return graphName+"_edgelabel_"+i},"dx":0,"dy":-5,"fill":function(d){return data.colors[d.type||"default"]}});newGraph.edgelabels.append("textPath").attr({"xlink:href":function(d,i){return"#"+graphName+"_edgepath_"+i},"startOffset":"50%","text-anchor":"middle"}).text(function(d,i){return d.text});this.addArrowheads(graphName,["default","relation"]);
if(htmlNode.offsetWidth>0||htmlNode.offsetHeight>0){setTimeout(function(){newGraph.svg.selectAll(".nodelabel").call(_this.wrap);newGraph.nodeLabelsInitialised=true},25);newGraph.force.start()}newGraph.force.on("tick",function(e){newGraph.nodes.each(_this.gravity(newGraph,.2*e.alpha)).each(_this.collide(newGraph,.5)).attr({"r":function(d){return _this.getBiggerSide(graphName)/100*(d.r/2)},"cx":function(d){return d.x},"cy":function(d){return d.y}});newGraph.edges.each(function(d){var altCoords=_this.offsetEdge(newGraph,
d);d3.select(this).attr({"x1":altCoords.x1,"y1":altCoords.y1,"x2":altCoords.x2,"y2":altCoords.y2})});newGraph.nodelabels.attr("transform",function(d){return"translate("+d.x+", "+(d.y-this.getBoundingClientRect().height/2)+")"});if(newGraph.nodeLabelsInitialised)newGraph.nodelabels.attr("style",function(d){return _this.resizeFont(graphName,d,"Node",true)});newGraph.edgepaths.attr("d",function(d){var altCoords=_this.offsetEdge(newGraph,d);return"M "+altCoords.x1+" "+altCoords.y1+" L "+altCoords.x2+
" "+altCoords.y2});newGraph.edgelabels.attr({"transform":function(d,i){if(d.target.x<d.source.x){var bbox=this.getBBox(),rx=bbox.x+bbox.width/2,ry=bbox.y+bbox.height/2;return"rotate(180 "+rx+" "+ry+")"}else return"rotate(0)"},"style":function(d){return _this.resizeFont(graphName,d,"Edge",true)}})})}};window.MNDMPS=window.MNDMPS||{};
