...
target:0,text:"isa"},{source:6,target:3,text:"isa",type:"relation"},{source:7,target:5,text:"isa",type:"relation"}]}];window.MNDMPS=window.MNDMPS||{};
window.MNDMPS.Slider={_data:{},processScroll:function(){var _this=window.MNDMPS.Slider,data=_this._data,knobPosition=data.containerPercent*data.wrapper.scrollLeft/data.scrolledPercent;console.log(knobPosition);data.scrollKnob.style.transform="translateX("+knobPosition+"px)"},init:function(){var data=this._data;data.scroller=document.getElementsByClassName("promoSlider")[0];data.container=data.scroller.children[0];data.wrapper=data.container.children[0];data.controls=data.scroller.children[1];data.scrollbar=
data.controls.children[0];data.scrollKnob=data.scrollbar.children[0];data.tabs=data.controls.children[1];data.scrollKnobWidth=100/data.tabs.children.length;data.scrollKnob.style.width=data.scrollKnobWidth+"%";data.scrollerWidth=data.scroller.offsetWidth-data.scrollKnob.offsetWidth;data.contentWidth=data.scroller.offsetWidth*data.tabs.children.length;data.containerPercent=Math.floor(data.scrollerWidth/100);data.scrolledPercent=Math.floor(data.contentWidth-data.scrollerWidth)/100;data.wrapper.addEventListener("scroll",
this.processScroll,false)}};window.MNDMPS=window.MNDMPS||{};
...
window.addEventListener("resize",function(){if(codeEditor){var tabs=codeEditor.getElementsByClassName("content")[0].children;for(var i=0;i<tabs.length;i++)if(tabs[i].classList.contains("active")){window.MNDMPS.Graph.redraw("graph"+i);break}}},false);window.MNDMPS.Slider.init()}};window.MNDMPS.App.init();
