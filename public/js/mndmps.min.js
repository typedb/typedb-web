...
event.cancelBubble=true;event.returnValue=false;return false},serializeObject:function(el){var o={},a=el.serializeArray();$.each(a,function(){if(o[this.name]!==undefined){if(!o[this.name].push)o[this.name]=[o[this.name]];o[this.name].push(this.value||"")}else o[this.name]=this.value||""});return o}};window.MNDMPS=window.MNDMPS||{};
...
window.MNDMPS.Nav={_data:{},toggleWhite:function(state){var data=this._data,action=state?"add":"remove";data.nav.classList[action]("white")},checkPosition:function(){var _this=window.MNDMPS.Nav,data=_this._data,scrolled=document.documentElement.scrollTop||document.body.scrollTop;if(scrolled>0)_this.toggleWhite(true);else _this.toggleWhite(false)},checkWidth:function(){var _this=window.MNDMPS.Nav,data=_this._data;if(window.innerWidth>=750)data.menu.classList.remove("active")},toggleMenu:function(event){var _this=
...
this.toggleMenu,false);window.addEventListener("scroll",this.checkPosition,false);window.addEventListener("resize",this.checkWidth,false);this.checkWidth();this.checkPosition()}};window.MNDMPS=window.MNDMPS||{};
...
tick)}tick()},initImplicitData:function(node){var width=node.offsetWidth,height=node.offsetHeight,svg=node.children[0],maskPath=svg.getElementsByClassName("maskPath")[0],maskVisible=svg.getElementsByClassName("maskVisible")[0],speed=4E3,timeout=600,pos={max:80,min:0},maskTween=new Tweenable;function moveMask(){var currentPos=maskVisible.getAttribute("data-x")||pos.min;currentPos=parseInt(currentPos,10);maskTween.tween({from:{x:currentPos>40?pos.max:pos.min},to:{x:currentPos>40?pos.min:pos.max},duration:speed,
...
this._data,codeEditor=document.getElementsByClassName("editor")[0];window.MNDMPS.Nav.init();this.typist();var codeSplitter=window.MNDMPS.Splitter.create(document.getElementById("code-container").querySelectorAll('[data-splitter="view"]')[0]);window.MNDMPS.Splitter.initAutomove(codeSplitter);window.MNDMPS.Splitter.init();window.MNDMPS.Tabs.init(document.getElementById("code-container").querySelectorAll('[data-tabs="view"]')[0]);this.initPrism();var graqlSvgContainers=document.querySelectorAll('[data-splitter="slides"]')[0].querySelectorAll('[data-slide="right"]');
for(var i=0;i<graqlSvgContainers.length;i++)if(window.MNDMPS.Graph._data.homepageGraphs[i])window.MNDMPS.Graph.init(graqlSvgContainers[i],window.MNDMPS.Graph._data.homepageGraphs[i]);window.addEventListener("resize",function(){window.MNDMPS.Map.center(window.MNDMPS.Map.data.googleMap,window.MNDMPS.Map.data.googleMapLatLng,0,0);if(codeEditor){var tabs=codeEditor.getElementsByClassName("content")[0].children;for(var i=0;i<tabs.length;i++)if(tabs[i].classList.contains("active")){window.MNDMPS.Graph.redraw("graph"+
i);break}}},false);window.MNDMPS.Slider.init();google.maps.event.addDomListener(window,"load",window.MNDMPS.Map.load);if(document.getElementsByClassName("modal-underlay").length)window.MNDMPS.Modal.init();var getMindmapsButton=document.getElementsByClassName("installMindmaps");if(getMindmapsButton.length)for(var i=0;i<getMindmapsButton.length;i++)getMindmapsButton[i].addEventListener("click",window.MNDMPS.Modal.open,false);this.initAdvantages()}};window.MNDMPS.App.init();
