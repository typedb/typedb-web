...
window.MNDMPS.Nav={_data:{},toggleWhite:function(state){var data=this._data,action=state?"add":"remove";data.nav.classList[action]("white")},checkPosition:function(){var _this=window.MNDMPS.Nav,data=_this._data,scrolled=document.body.scrollTop;if(scrolled>0)_this.toggleWhite(true);else _this.toggleWhite(false)},checkWidth:function(){var _this=window.MNDMPS.Nav,data=_this._data;if(window.innerWidth>=750)data.menu.classList.remove("active")},toggleMenu:function(event){var _this=window.MNDMPS.Nav,data=
_this._data,node=event.target,action=data.menu.classList.contains("active")?"remove":"add";if(node.classList.contains("toggle")||node.classList.contains("menu"))data.menu.classList[action]("active");if(node.classList.contains("toggle"))event.stopPropagation()},init:function(){var data=this._data;data.nav=document.getElementsByClassName("nav")[0];data.menu=document.getElementsByClassName("menu")[0];data.toggle=data.nav.getElementsByClassName("toggle")[0];data.menu.addEventListener("click",this.toggleMenu,
false);window.addEventListener("scroll",this.checkPosition,false);window.addEventListener("resize",this.checkWidth,false);this.checkWidth();this.checkPosition()}};window.MNDMPS=window.MNDMPS||{};
...
window.MNDMPS.App={_data:{},typist:function(){var data=this._data;data.typist=$("#typist");data.typist.typed({strings:["a knowledge graph","an inference engine","an analytics tool","a distributed semantic database"],typeSpeed:10,backSpeed:5,backDelay:1500,loop:false})},initPrism:function(){var textareas=document.getElementsByClassName("code-input");function renderOutput(input){var value=input.value;value=value.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")+"\n";$(input).next().children(0)[0].innerHTML=
value;Prism.highlightAll()}function listenerForScroll(input){input.addEventListener("scroll",function(event){$(this).next()[0].scrollTop=this.scrollTop},false)}for(var i=0;i<textareas.length;i++){renderOutput(textareas[i]);listenerForScroll(textareas[i])}},init:function(){var data=this._data,codeEditor=document.getElementsByClassName("editor")[0];window.MNDMPS.Nav.init();this.typist();var codeSplitter=window.MNDMPS.Splitter.create(document.getElementById("code-container").querySelectorAll('[data-splitter="view"]')[0]);
...
var getMindmapsButton=document.getElementsByClassName("installMindmaps");if(getMindmapsButton.length)for(var i=0;i<getMindmapsButton.length;i++)getMindmapsButton[i].addEventListener("click",window.MNDMPS.Modal.open,false)}};window.MNDMPS.App.init();
