...
window.MNDMPS.App={_data:{},typist:function(){var data=this._data;data.typist=[];var topTypist=$("#topTypist");topTypist.typed({strings:["a knowledge graph","an inference engine","an analytics tool","a distributed semantic database"],typeSpeed:10,backSpeed:5,backDelay:1500,loop:false});data.typist.push(topTypist);var advantagesTypist=$("#advantagesTypist");advantagesTypist.typed({strings:['$z isa movie value "Avatar"',"select $x(id, has title)","($x, $y) isa casting",'$x isa movie value "Big Fish"'],
typeSpeed:10,backSpeed:5,backDelay:1500,loop:true});data.typist.push(advantagesTypist)},initPrism:function(){var textareas=document.getElementsByClassName("code-input");function renderOutput(input){var value=input.value;value=value.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")+"\n";$(input).next().children(0)[0].innerHTML=value;Prism.highlightAll()}function listenerForScroll(input){input.addEventListener("scroll",function(event){$(this).next()[0].scrollTop=this.scrollTop},false)}
for(var i=0;i<textareas.length;i++){renderOutput(textareas[i]);listenerForScroll(textareas[i])}},initRealTimeAnalytics:function(node){var i=null,tempMaxArray=[],tickGap=50,segments=80,duration=1E3,nowDate=new Date(Date.now()-duration),data=[d3.range(segments).map(function(){return 0}),d3.range(segments).map(function(){return 0})],margin={top:2,right:1,bottom:20,left:1},width=node.offsetWidth-margin.right-margin.left,height=node.offsetHeight-margin.top-margin.bottom,x=d3.time.scale().domain([nowDate-
