...
d3.xml=d3_xhrType(function(request){return request.responseXML});if(typeof define==="function"&&define.amd)this.d3=d3,define(d3);else if(typeof module==="object"&&module.exports)module.exports=d3;else this.d3=d3}();(function(){var _symbol=d3.svg.symbol(),_line=d3.svg.line();d3.superformula=function(){var type=_symbol.type(),size=_symbol.size(),segments=size,params={};function superformula(d,i){var n,p=_superformulaTypes[type.call(this,d,i)];for(n in params)p[n]=params[n].call(this,d,i);return _superformulaPath(p,segments.call(this,d,i),Math.sqrt(size.call(this,d,i)))}superformula.type=function(x){if(!arguments.length)return type;type=d3.functor(x);return superformula};superformula.param=function(name,value){if(arguments.length<
2)return params[name];params[name]=d3.functor(value);return superformula};superformula.size=function(x){if(!arguments.length)return size;size=d3.functor(x);return superformula};superformula.segments=function(x){if(!arguments.length)return segments;segments=d3.functor(x);return superformula};return superformula};function _superformulaPath(params,n,diameter){var i=-1,dt=2*Math.PI/n,t,r=0,x,y,points=[];while(++i<n){t=params.m*(i*dt-Math.PI)/4;t=Math.pow(Math.abs(Math.pow(Math.abs(Math.cos(t)/params.a),
params.n2)+Math.pow(Math.abs(Math.sin(t)/params.b),params.n3)),-1/params.n1);if(t>r)r=t;points.push(t)}r=diameter*Math.SQRT1_2/r;i=-1;while(++i<n){x=(t=points[i]*r)*Math.cos(i*dt);y=t*Math.sin(i*dt);points[i]=[Math.abs(x)<1E-6?0:x,Math.abs(y)<1E-6?0:y]}return _line(points)+"Z"}var _superformulaTypes={asterisk:{m:12,n1:.3,n2:0,n3:10,a:1,b:1},bean:{m:2,n1:1,n2:4,n3:8,a:1,b:1},butterfly:{m:3,n1:1,n2:6,n3:2,a:.6,b:1},circle:{m:4,n1:2,n2:2,n3:2,a:1,b:1},clover:{m:6,n1:.3,n2:0,n3:10,a:1,b:1},cloverFour:{m:8,
n1:10,n2:-1,n3:-8,a:1,b:1},cross:{m:8,n1:1.3,n2:.01,n3:8,a:1,b:1},diamond:{m:4,n1:1,n2:1,n3:1,a:1,b:1},drop:{m:1,n1:.5,n2:.5,n3:.5,a:1,b:1},ellipse:{m:4,n1:2,n2:2,n3:2,a:9,b:6},gear:{m:19,n1:100,n2:50,n3:50,a:1,b:1},heart:{m:1,n1:.8,n2:1,n3:-8,a:1,b:.18},heptagon:{m:7,n1:1E3,n2:400,n3:400,a:1,b:1},hexagon:{m:6,n1:1E3,n2:400,n3:400,a:1,b:1},malteseCross:{m:8,n1:.9,n2:.1,n3:100,a:1,b:1},pentagon:{m:5,n1:1E3,n2:600,n3:600,a:1,b:1},rectangle:{m:4,n1:100,n2:100,n3:100,a:2,b:1},roundedStar:{m:5,n1:2,n2:7,
n3:7,a:1,b:1},square:{m:4,n1:100,n2:100,n3:100,a:1,b:1},star:{m:5,n1:30,n2:100,n3:100,a:1,b:1},triangle:{m:3,n1:100,n2:200,n3:200,a:1,b:1}};d3.superformulaTypes=d3.keys(_superformulaTypes)})();function atvImg(){var d=document,de=d.documentElement,bd=d.getElementsByTagName("body")[0],win=window,imgs=d.querySelectorAll(".atvImg"),totalImgs=imgs.length,supportsTouch="ontouchstart"in win||navigator.msMaxTouchPoints,thresholds=true,offsetThreshold=.6,angleThreshold=140;if(totalImgs<=0)return;for(var l=0;l<totalImgs;l++){var thisImg=imgs[l],layerElems=thisImg.querySelectorAll(".atvImg-layer"),totalLayerElems=layerElems.length;if(totalLayerElems<=0)continue;while(thisImg.firstChild)thisImg.removeChild(thisImg.firstChild);
...
lookbehind:true}});Prism.languages.insertBefore("java","function",{"annotation":{alias:"punctuation",pattern:/(^|[^.])@\w+/,lookbehind:true}});(function(){Prism.languages.graql={comment:{pattern:/#.*/,alias:"comment"},string:{pattern:/".*?"/,alias:"string"},keyword:{pattern:/(match|ask|insert|delete|select|isa|ako|plays-role|has-role|has-scope|datatype|is-abstract|has|value|id|of|limit|offset|order|by)(?![-\$a-zA-Z_0-9])/,alias:"keyword"},special:{pattern:/graql>|results>|\.\.\./},month:{pattern:/(?!{-\$a-zA-Z_0-9})/,alias:"number"},variable:{pattern:/\$[-a-zA-Z_0-9]+(?![-\$a-zA-Z_0-9])/,alias:"variable"},type:{pattern:/[-a-zA-Z_][-a-zA-Z_0-9]*(?![-\$a-zA-Z_0-9])/,
...
this.toggleMenu,false);window.addEventListener("resize",this.checkWidth,false);this.checkWidth();if(document.getElementById("code-container")){window.addEventListener("scroll",this.checkPosition,false);this.checkPosition()}else this.toggleWhite(true)}};window.MNDMPS=window.MNDMPS||{};
...
window.MNDMPS.Careers={_data:{},init:function(node){var data=this._data,domParser=new DOMParser,loading=node.getElementsByClassName("loading")[0];data.jobsBlock=node.children[0].children[0];function kFormatter(num){num=parseInt(num,10);var newNum=num>999?Math.floor(num/1E3):num;return newNum+"K"}function timeFormatter(date){var monthNames=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];var date=new Date(date),day=date.getDate(),monthIndex=date.getMonth(),year=date.getFullYear();
return newDate=day+" "+monthNames[monthIndex]+" "+year}function generateJob(obj){var jobBlock=document.createElement("div");jobBlock.classList.add("job");jobBlock.innerHTML="<h3>"+obj.title+'</h3>                <div class="text-block columned two">                    <p>'+obj.description.replace(/[0-9]+\.[^.]*\.(?!\s*[0-9])/g,"$&<br>").replace(/[0-9]+\./g,"<br>$&").replace(/\*/g,"<br>")+"</p>                </div>                <ul>                    <li>Updated on: "+timeFormatter(obj.updated_at)+
"</li>                    <li>Remote: "+(obj.remote_ok?"Yes":"No")+'</li>                    <li>Type: <span class="capitalise">'+obj.job_type+"</span></li>                    <li>Compensation: &pound;"+kFormatter(obj.salary_min)+" \u2014 &pound;"+kFormatter(obj.salary_max)+"</li>                    <li>Equity: "+obj.equity_min+"% \u2014 "+obj.equity_max+'%</li>                </ul>                <div class="bottomLinks">                    <a href="mailto:recruitment@mindmaps.io">Apply</a> or <a target="_blank" rel="nofollow" href="'+
obj.angellist_url+'">Check on AngelList</a>                </div>';return jobBlock}window.MNDMPS.Ajax.getAngelJobs().then(function(response){response=JSON.parse(response);data.jobsBlock.removeChild(loading);for(var i=0;i<response.jobs.length;i++)data.jobsBlock.appendChild(generateJob(response.jobs[i]))},function(error){console.log(error)})}};window.MNDMPS=window.MNDMPS||{};
...
for(var i=0;i<images.length;i++)images[i].src="/docs/"+data.type+"/images/"+images[i].getAttribute("src");return wrapper.innerHTML},refreshPage:function(html){var data=this._data;data.view.innerHTML=html},init:function(node){var data=this._data;data.wapper=node;data.type=data.wapper.getAttribute("data-type");data.nav=node.getElementsByClassName("docs-menu")[0];data.view=node.getElementsByClassName("docs-content")[0];data.nav.addEventListener("click",this.getPage,false);window.MNDMPS.Ajax.getDocsMenu(data.type).then(function(response){data.nav.innerHTML=
response},function(error){console.log(error)})}};window.MNDMPS=window.MNDMPS||{};
...
easing:"easeInOutQuart",step:function(state){maskVisible.setAttribute("data-x",state.x);maskVisible.style.transform="translate("+state.x+"px, 0px)";maskPath.style.transform="translate("+state.x+"px, 0px)"},finish:function(){setTimeout(function(){moveMask()},timeout)}})}moveMask()},initAdvantages:function(){this.initRealTimeAnalytics(document.getElementsByClassName("advantages-realtime")[0]);this.initImplicitData(document.getElementsByClassName("advantages-discover")[0])},init:function(){var data=
