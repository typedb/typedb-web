...
export function renderVertex(
    ...
    switch (vertex.encoding) {
        case "entity":
            ...
            vertex.gfx.hitArea = new PIXI.RoundedRectangle(
                -vertex.width / 2,
                -vertex.height / 2,
                vertex.width,
                vertex.height,
...
export function renderVertexLabel(
    vertex: Renderer.Vertex,
    useFallbackFont: boolean,
    theme: GraphVisualisationTheme,
    isMobile: boolean
    ...
    const text1 = new PIXI.Text(vertex.label, {
        fontSize: defaultStyles.vertexLabel.fontSize * (isMobile ? 0.54 : 1),
...
export function renderEdgeLabel(
    ...
    const text1 = new PIXI.Text(edge.label, {
        fontSize: defaultStyles.edgeLabel.fontSize * (isMobile ? 0.54 : 1),
        fontFamily,
    ...
    switch (target.encoding) {
        ...
        case "relation":
            const targetRect: Rect = {
                x: target.x - target.width / 2 - 4,
                y: target.y - target.height / 2 - 4,
                w: target.width + 8,
                h: target.height + 8,
            };
        ...
        case "attribute":
            const targetEllipse: Ellipse = {
                x: target.x,
                y: target.y,
                hw: target.width / 2 + 2,
                hh: target.height / 2 + 2,
            };
            return ellipseIncomingLineIntersect(source as Point, targetEllipse);
    }
}

function createD3ForceSimulation(
...
export function renderGraph(
    ...
    const sizeModifier = isMobile ? 0.54 : 1;
    const vertices: Renderer.Vertex[] = graphData.vertices.map((d) =>
        Object.assign({}, d, {
