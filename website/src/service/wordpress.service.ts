...
import {
    ApplicationArticle, applicationArticleSchemaName, Article, articleFromApi, BlogCategoryID, BlogFilter,
    blogNullFilter, BlogPost, blogPostSchemaName, FundamentalArticle, fundamentalArticleSchemaName, SanityArticle,
    SanityBlogPost, WordpressPost, WordpressPosts
...
export class WordpressService {
    ...
    constructor(
        ...
        this.fundamentalArticles = this.transferState.useScullyTransferState("fundamentalArticles", this.listFundamentalArticles()).pipe(
            filter((data) => !!data?.length),
            shareReplay(),
        );
        this.applicationArticles = this.transferState.useScullyTransferState("applicationArticles", this.listApplicationArticles()).pipe(
            filter((data) => !!data?.length),
            shareReplay(),
        );
    ...
    private listArticles<T extends SanityArticle, U extends Article>(schemaName: string, limit = 100, offset = 0): Observable<U[]> {
        return combineLatest([this.contentService.data, this.listWordpressPosts(limit, offset)]).pipe(map(([data, wpPosts]) => {
            return data.getDocumentsByType<T>(schemaName)
                .map(sanityPost => [sanityPost, wpPosts.find(wpPost => wpPost.slug === sanityPost.slug.current)] as [SanityArticle, WordpressPost])
                .filter(([_sanityPost, wpPost]) => !!wpPost)
